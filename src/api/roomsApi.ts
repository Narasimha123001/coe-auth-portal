...
export const roomsApi = {
  ...
  removeAccess: async (
    ...
    await api.delete(
      `/v1/blackRoom/remove/${registerNumber}/${encodeURIComponent(roomName)}`,
    );
  },
  ...
  getExamRooms: async (
    ...
    const response = await api.get("/v1/examrooms/all", {
      params: { page, size, search, sort },
    });
    return response.data;
  },

  createExamRoom: async (data: RoomRequest): Promise<ExamRoom> => {
  ...
  updateExamRoom: async (
    ...
    const response = await api.put(`/v1/examrooms/${roomId}`, data);
    return response.data;
  },

  deleteExamRoom: async (roomNumber: number): Promise<void> => {
